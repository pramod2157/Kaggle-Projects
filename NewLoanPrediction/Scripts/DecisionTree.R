...
dt <- rpart(NewTrain_Y ~ .,data = NewTrain[,-1] ,  method = )

pred <- predict(dt,NewTest[,-1],type =)
pred
pred <- ifelse(pred>1.5, 'Y', 'N')
submission <- data.frame(Loan_ID = NewTest$Loan_ID, Loan_Status = pred)
write.csv(submission, 'F:/KaggleProjectsAndData/AnalyticsVidhya/NewLoanPrediction/Submissions/DTProb.csv', row.names=FALSE, quote = FALSE)
#Accuracy  0.770833333333

#---------------With Classification--------------------------

NewTrain <- cbind(NewTrain_X, NewTrain_Y)
dt <- rpart(NewTrain_Y ~ ApplicantIncome + Credit_History,data = NewTrain[,-1] ,  method = 'class')

pred <- predict(dt,NewTest[,-1],type ='class')
summary(pred)
pred <- ifelse(pred==2, 'Y', 'N')
pred <- as.factor(pred)
submission <- data.frame(Loan_ID = NewTest$Loan_ID, Loan_Status = pred)
write.csv(submission, 'F:/KaggleProjectsAndData/AnalyticsVidhya/NewLoanPrediction/Submissions/DTClass.csv', row.names=FALSE, quote = FALSE)
