...
library(e1071)
naiveBayesModel <- naiveBayes(NewTrain_Y ~ .,data = NewTrain[,-1])

pred <- predict(naiveBayesModel,NewTest[,-1],type = 'raw')
pred
pred <- ifelse(pred[,2]>pred[,1], 'Y', 'N')

submission <- data.frame(Loan_ID = NewTest$Loan_ID, Loan_Status = pred)
write.csv(submission, 'F:/KaggleProjectsAndData/AnalyticsVidhya/NewLoanPrediction/Submissions/NaiveBayes.csv', row.names=FALSE, quote = FALSE)
#Accuracy 0.75