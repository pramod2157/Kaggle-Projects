...
SVMmodel <- svm(NewTrain_Y ~ ., data = NewTrain[,-1], type = 'C')

pred <- predict(SVMmodel,NewTest,type = 'Class')

#pred <- ifelse(pred==2, 'Y', 'N')

submission <- data.frame(Loan_ID = NewTest$Loan_ID, Loan_Status = pred)
write.csv(submission, 'F:/KaggleProjectsAndData/AnalyticsVidhya/NewLoanPrediction/Submissions/svm.csv', row.names=FALSE, quote = FALSE)
